...
class OptCamera {
  public:
	...
	virtual void handleInput_opt(int dx, int dy, bool lmbDown, bool rmbDown, int modifierKeys) = 0;

	/// <summary>
	/// Called by the lab manager when the displayed object changes
	/// </summary>
	virtual void displayedObjectChanged_opt() = 0;
...
class OrbitCameraOpt : public OptCamera {
  public:
	OrbitCameraOpt() : OptCamera(cam_create("Lab orbit camera")) {}
	...
	void handleInput_opt(int dx, int dy, bool /*lmbDown*/, bool rmbDown, int modifierKeys) override;

	void displayedObjectChanged_opt() override;
