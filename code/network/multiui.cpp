...
void multi_join_game_init()
{
	...
	if ( Cmdline_start_netgame ) {
		...
		const auto number_end = Cmdline_connect_addr.find(':');
		if ( number_end != SCP_string::npos ) {
			port_num = (short)atoi(Cmdline_connect_addr.substr(0, number_end).c_str());
		}
		ip_addr = inet_addr(Cmdline_connect_addr.c_str());
