...
std::tuple<size_t, size_t, bool> split_str_once(const char *src, int max_pixel_w, size_t max_line_len, float scale, int *split_width, int *split_height)
{
	...
	*split_width = 0;	// for the first iteration of the loop
	...
	// the whole string fit on one line, so no split is needed
	if (iter == range_end)
	{
		// if the string ends in grayspace, trim it
		if (last_was_gray)
		{
	...
	// we didn't fit (we broke out of the loop early on width or length) and there's no natural break to use, so force a split here
	if (!found_break)
	{
	...
	// from here on we know found_break is true, so saved_iter/saved_len point at a real break
