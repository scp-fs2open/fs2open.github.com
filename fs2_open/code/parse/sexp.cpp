...
//   Operator, Identity, Min / Max arguments
	...
	{ "set-arrival-info",				OP_SET_ARRIVAL_INFO,					2,	6,			SEXP_ACTION_OPERATOR	},	// Goober5000
	{ "set-departure-info",				OP_SET_DEPARTURE_INFO,					2,	5,			SEXP_ACTION_OPERATOR	},	// Goober5000
	...
	{ "deselect-nav",					OP_NAV_DESELECT,						0,	0,			SEXP_ACTION_OPERATOR	}, //Talon1024
	...
	if (Sexp_nodes[cur_node].first != -1) {
		...
		if (Sexp_nodes[cur_node].iteration != -1) {
			...
			if (Sexp_nodes[cur_node].flags & SNF_CONTAINER_FINAL_ITERATION) {
				...
				Sexp_nodes[cur_node].iteration = -1;
			}		
			// recurse
			else {	
		...
		switch (actual_flag) {
			...
			case SNF_CONTAINER_ITERATE_FORWARDS:
				...
				if (Sexp_nodes[n].iteration + 1 == (int)Sexp_list_containers[index].data.size()) {
					...
					Sexp_nodes[n].iteration = -1;		// reset, might be used in a repeating event!
				}
