...
{	
	...
	for ( so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so) ) {		
		...
		if (is_big_ship(&Ship_info[Ships[target->instance].ship_info_index]) || is_huge_ship(&Ship_info[Ships[target->instance].ship_info_index])) {
...
void emp_process_ship(ship *shipp)
{
	...
	if (!(Ship_info[shipp->ship_info_index].flags[Ship::Info_Flags::Fighter] || Ship_info[shipp->ship_info_index].flags[Ship::Info_Flags::Bomber])){
		return;
	}
