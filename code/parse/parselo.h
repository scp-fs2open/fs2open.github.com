...
template< size_t src_size, size_t n_chars_size, size_t p_str_size > inline 
int split_str(char (&src)[ src_size ], int max_pixel_w, int (&n_chars)[ n_chars_size ], char * (&p_str)[ p_str_size ], int max_lines, char ignore_char = -1)
{
	return split_str((const char*)src, max_pixel_w, (int*)n_chars, (const char**)p_str, max_lines, ignore_char);
}
