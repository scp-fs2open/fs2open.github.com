...
int sexp_container::size() const
{
	if (is_list()) {
		...
		Assertion(false, "Unknown container type %d", static_cast<int>(type));
		return 0;
	}
}
...
const SCP_string &sexp_container::get_value_at_index(int index) const
{
	...
	if (is_list()) {
		...
		Assertion(false, "Container %s has invalid type (%d). Please report!", container_name.c_str(), static_cast<int>(type));
...
SCP_string sexp_container::apply_list_modifier(ListModifier modifier, int data_index)
{
	...
	switch (modifier) {
		...
		default:
			Assertion(false, "Unhandled list modifier %d. Please report!", static_cast<int>(modifier));
			return "";
	}
}
...
bool sexp_container_CTEXT_helper(int &node, sexp_container &container, SCP_string &result)
{
	if (container.is_map()) {
		...
		Assertion(false, "Container %s has invalid type (%d). Please report!",
			container.container_name.c_str(),
...
int sexp_is_container_empty(int node)
{
	...
	if (container.is_map()) {
		...
		Assertion(false, "Container %s has invalid type (%d). Please report!", container_name, static_cast<int>(container.type));
	}
...
int sexp_container_eval_status_sexp(int op_num, int node)
{
	switch (op_num) {
		...
		default:
			Assertion(false, "Unknown container status SEXP operator %d. Please report!", op_num);
			return SEXP_FALSE;
	}
}
...
void sexp_copy_container(int node)
{
	...
	if (src_container.is_list()) {
		...
		Assertion(false, "Container %s has invalid type (%d). Please report!", src_container_name, static_cast<int>(src_container.type));
	}
}
...
int sexp_container_eval_change_sexp(int op_num, int node)
{
	...
	default:
		Assertion(false, "Unknown container change SEXP operator %d", op_num);
		return SEXP_FALSE;
	}
}
