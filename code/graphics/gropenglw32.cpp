...
/*
 * $Logfile: /Freespace2/code/Graphics/GrOpenGLW32.cpp $
 * $Revision: 1.1 $
 * $Date: 2002-10-11 21:30:00 $
 * $Author: phreak $
 *
...
//  				x1, x2, y));
	// FIXME
	gr_opengl_line_internal(i2fl(x1), i2fl(y), i2fl(x2), i2fl(y));
}
...
{
	// FIXME
	gr_opengl_line_internal(i2fl(x1), i2fl(y1), i2fl(x2), i2fl(y2));
}
...
void gr_opengl_fog_set(int fog_mode, int r, int g, int b, float near, float far)
{
	...
	mprintf(("ANOTHER TODO: GL_fog_coord_EXT"));
...
void gr_opengl_get_pixel(int x, int y, int *r, int *g, int *b)
{
	...
	glReadPixels(x,gr_screen.max_h-y,1,1,GL_RGB, GL_UNSIGNED_BYTE, color);
