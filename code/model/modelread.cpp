...
//				sm->data_offset = cfread_int(fp);	//offset
				...
				if (sm->flags[Model::Submodel_flags::Is_thruster] &&
					!(sm->flags[Model::Submodel_flags::No_collisions] ||
						sm->flags[Model::Submodel_flags::Nocollide_this_only] ||
						sm->flags[Model::Submodel_flags::Collide_invisible]))
				{
		...
		for (j=0; j<pm->n_models; j++ )	{
			...
			if (ndiff==1)	{		// They only differ by one character!
				...
				if (sm1->parent == -1 && sm2->parent == -1 &&
					!(sm1->flags[Model::Submodel_flags::Is_damaged] ||
						sm1->flags[Model::Submodel_flags::Is_live_debris]) &&
					!(sm2->flags[Model::Submodel_flags::Is_damaged] ||
						sm2->flags[Model::Submodel_flags::Is_live_debris])) {
					dl2 = dl2 - dl1;
					dl1 = 0;
				}
