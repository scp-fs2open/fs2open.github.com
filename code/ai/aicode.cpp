...
void ai_do_objects_repairing_stuff( object *repaired_objp, object *repair_objp, int how )
{
	...
	case REPAIR_INFO_BEGIN:
		...
		if ( repaired_objp->flags & OF_PLAYER_SHIP ) {
			if ( !(Game_mode & GM_MULTIPLAYER) ) {
				Player->stats.m_score -= (int)(The_mission.ai_profile->repair_penalty[Game_skill_level] * scoring_get_scale_factor());			// subtract the penalty
			} else {
				/*
				int pnum;
				...
				if ( pnum != -1 ) {
					Net_players[pnum].player->stats.m_score -= (int)(The_mission.ai_profile->repair_penalty[Game_skill_level] * scoring_get_scale_factor());

					// squad war
					multi_team_maybe_add_score(-(int)(The_mission.ai_profile->repair_penalty[Game_skill_level] * scoring_get_scale_factor()), Net_players[pnum].p_info.team);
				} else {
