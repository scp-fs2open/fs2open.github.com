...
static float shipfx_calculate_effect_radius( object *objp, WarpDirection warp_dir )
{
	...
	else
	{
		...
		if(warp_dir == WarpDirection::WD_WARP_IN && sip->warpin_radius > 0.0f)
		{
			return sip->warpin_radius;
		}
		else if(warp_dir == WarpDirection::WD_WARP_OUT && sip->warpout_radius > 0.0f)
		{
...
{
	//Warpin time defined
	if( (warp_dir == WarpDirection::WD_WARP_IN) && (sip->warpin_time > 0.0f)) {
		return (float)sip->warpin_time/1000.0f;
	//Warpout time defined
	} else if( (warp_dir == WarpDirection::WD_WARP_OUT) && (sip->warpout_time > 0.0f)) {
		return (float)sip->warpout_time/1000.0f;
	//Warpin defined
	} else if ( (warp_dir == WarpDirection::WD_WARP_IN) && (sip->warpin_speed != 0.0f) ) {
		return warping_dist / sip->warpin_speed;
	//Warpout defined
	} else if ( (warp_dir == WarpDirection::WD_WARP_OUT) && (sip->warpout_speed != 0.0f) ) {
		return warping_dist / sip->warpout_speed;
	//Player warpout defined
	} else if ( (warp_dir == WarpDirection::WD_WARP_OUT) && (objp == Player_obj) && (sip->warpout_player_speed != 0.0f) ) {
		return warping_dist / sip->warpout_player_speed;
	//Player warpout not defined
	} else if ( (warp_dir == WarpDirection::WD_WARP_OUT) && (objp == Player_obj) ) {
...
{
	objp = NULL;
	direction = WarpDirection::WD_NONE;
	shipp = NULL;
	sip = NULL;
}
...
int WE_Default::warpStart()
{
	...
	if(direction == WarpDirection::WD_WARP_OUT && objp == Player_obj)
	{
	...
	portal_objp = nullptr;
	if ((direction == WarpDirection::WD_WARP_IN) && shipfx_special_warp_objnum_valid(shipp->special_warpin_objnum))
	{
		portal_objp = &Objects[shipp->special_warpin_objnum];
	}
	else if ((direction == WarpDirection::WD_WARP_OUT) && shipfx_special_warp_objnum_valid(shipp->special_warpout_objnum))
	{
	...
	else if (direction == WarpDirection::WD_WARP_IN)
	{
		// select the fireball we use
		int fireball_type = FIREBALL_WARP;
	...
	else if(direction == WarpDirection::WD_WARP_OUT)
	{
        shipp->flags.set(Ship::Ship_Flags::Depart_warp);
...
int WE_BSG::warpShipClip(model_render_params *render_info)
{
	...
	if(direction == WarpDirection::WD_WARP_OUT && stage > 0)
	{
		vec3d position;
