...
void message_log_add_segs(const char *text, int msg_color, int flags, SCP_vector<log_text_seg> *entry, bool split_string)
{
	...
	if (split_string) {
		...
		while (true) {
			...
			// we have more text to write (since we didn't return, above)
			if (split_len == 0) {
				// nothing fit on this line; if we can't advance either, we're stuck, so bump to a fresh line and try again
				if (split_next_pos == 0) {
					sanity_counter++;
					if (sanity_counter > 5) {
				...
				// note that this may be a forced mid-word split, in which case we simply continue splitting on the next line
