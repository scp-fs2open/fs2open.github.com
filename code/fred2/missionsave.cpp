	...
	for (i=0; i<Num_teams; i++) {
		...
		for (j=0; j<Team_data[i].num_ship_choices; j++) {
			...
			if (Team_data[i].ship_list_variables[j] != -1) {
				...
				fout_version("\t@%s\t", Sexp_variables[Team_data[i].ship_list_variables[j]].variable_name);
			}
			else {
				fout_version("\t\"%s\"\t", Ship_info[Team_data[i].ship_list[j]].name); 
			}
		...
		for (j=0; j<Team_data[i].num_weapon_choices; j++) {
			...
			if (Team_data[i].weaponry_pool_variable[j] != -1) {
				...
				fout_version("\t@%s\t", Sexp_variables[Team_data[i].weaponry_pool_variable[j]].variable_name); 
			}
			else {
				fout_version("\t\"%s\"\t", Weapon_info[Team_data[i].weaponry_pool[j]].name);
			}
