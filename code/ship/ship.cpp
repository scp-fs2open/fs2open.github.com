	...
	if (sip->flags & SIF_AFTERBURNER) {
		...
		for (int h = 0; h < pm_orig->n_thrusters; h++) {
			...
			//	ci = &sip->ct_info[sip->ct_count++];
				...
				if (pm_orig->thrusters[h].points[j].norm.xyz.z > -0.5)
					continue; // only make ab trails for thrusters that are pointing backwards

				ci->pt = pm_orig->thrusters[h].points[j].pnt;//offset
				ci->w_start = pm_orig->thrusters[h].points[j].radius * sip->afterburner_trail_width_factor;	// width * table loaded width factor
...
void ship_page_out_model_textures(int modelnum, int ship_index)
{
	...
	// afterburner
	if (sip->afterburner_trail.bitmap_id > -1) {
	...
	if (sip->thruster_secondary_glow_info.normal.bitmap_id > -1) {
		bm_page_out(sip->thruster_secondary_glow_info.normal.bitmap_id);
	}

	if (sip->thruster_secondary_glow_info.afterburn.bitmap_id > -1) {
		bm_page_out(sip->thruster_secondary_glow_info.afterburn.bitmap_id);
	}

	if (sip->thruster_tertiary_glow_info.normal.bitmap_id > -1) {
		bm_page_out(sip->thruster_tertiary_glow_info.normal.bitmap_id);
	}

	if (sip->thruster_tertiary_glow_info.afterburn.bitmap_id > -1) {
	...
	// thruster/particle bitmaps
	for (i = 0; i < (int)sip->normal_thruster_particles.size(); i++) {
		if (sip->normal_thruster_particles[i].thruster_bitmap.first_frame > -1)
			bm_page_out(sip->normal_thruster_particles[i].thruster_bitmap.first_frame);
	}

	for (i = 0; i < (int)sip->afterburner_thruster_particles.size(); i++) {
		if (sip->afterburner_thruster_particles[i].thruster_bitmap.first_frame > -1)
