...
void labviewer_setup_subsys_rotation()
{
	...
	if ( Ship_info[Lab_selected_index].subsystems.empty() ) {
		return;
	}


	uint n_subsystems = Ship_info[Lab_selected_index].subsystems.size();
	...
	Lab_ship_model_subsys = Ship_info[Lab_selected_index].subsystems;
}
	...
	if (change_model) {
		...
		if (Lab_model_num != -1) {
			...
			if (Lab_last_selected_ship >= 0) {
				ship_page_out_texture_set(Lab_last_selected_ship, true);
		...
		if (model_fname != NULL) {
			...
			Lab_model_num = model_load(model_fname, (valid_ship) ? Ship_info[sel_index].subsystems.size() : 0, subsystems, 0);
			...
			if (valid_ship) {
				if (Lab_model_num >= 0) {
					...
					ship_init_texture_set(sel_index, model_get(Lab_model_num));
				}

				for (uint idx = 0; idx < Ship_info[sel_index].subsystems.size(); idx++) {
