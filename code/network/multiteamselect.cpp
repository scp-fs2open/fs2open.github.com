	...
	for(idx=0;idx<MULTI_TS_MAX_TVT_TEAMS;idx++){
		for(s_idx=0;s_idx<MULTI_TS_NUM_SHIP_SLOTS;s_idx++){
			...
			if((Multi_ts_team[idx].multi_ts_player[s_idx] != NULL) && !MULTI_CONNECTED((*Multi_ts_team[idx].multi_ts_player[s_idx]))){
				...
				Objects[Multi_ts_team[idx].multi_ts_objnum[s_idx]].flags.unset(Object::Object_Flags::Player_ship);
				Objects[Multi_ts_team[idx].multi_ts_objnum[s_idx]].flags.unset(Object::Object_Flags::Could_be_player);
			}
		}
	}
}
...
int multi_ts_swap_player_player(int from_index,int to_index,int *sound,int player_index)
{
	...
	// update ship flags
	Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[to_index]].flags.unset(Object::Object_Flags::Could_be_player);
	Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[to_index]].flags.unset(Object::Object_Flags::Player_ship);
	if(Multi_ts_team[pl->p_info.team].multi_ts_player[to_index] != NULL){		
		Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[to_index]].flags.set(Object::Object_Flags::Player_ship);
	}		
	Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[from_index]].flags.unset(Object::Object_Flags::Could_be_player);
	Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[from_index]].flags.unset(Object::Object_Flags::Player_ship);
	if(Multi_ts_team[pl->p_info.team].multi_ts_player[from_index] != NULL){		
...
int multi_ts_ok_to_commit()
{
	...
	for(idx=0;idx<MULTI_TS_NUM_SHIP_SLOTS;idx++){
		...
		if( (Multi_ts_team[Net_player->p_info.team].multi_ts_objnum[idx] != -1) && 
			...
			if (!primary_ok && !(The_mission.ai_profile->flags[AI::Profile_flags::Multi_allow_empty_primaries])) {
				return 2;
			} 
...
void process_pslot_update_packet(ubyte *data, header *hinfo)
{
	...
	case TS_CODE_PLAYER_UPDATE:			
		...
		while(stop != 0xff){
			...
			GET_DATA(val);
			Objects[objnum].flags.unset(Object::Object_Flags::Player_ship);
			Objects[objnum].flags.unset(Object::Object_Flags::Could_be_player);
			switch(val){
			case 1 :
				Objects[objnum].flags.set(Object::Object_Flags::Player_ship);
				break;
			case 2 :
				Objects[objnum].flags.set(Object::Object_Flags::Could_be_player);
				break;
			}
