	...
	while (node != -1) {
		...
		switch (type) {
			...
			case OPF_ORDER_RECIPIENT:
				...
				// "<any IFF>" is fine for teams
				if ((type == OPF_SHIP_WING_TEAM || type == OPF_SHIP_WING_TEAM_POINT) && sexp_determine_team(CTEXT(node)) >= 0)	{
					break;
						}

				// only other possibility is waypoints
				if (type == OPF_SHIP_WING_TEAM_POINT || type == OPF_SHIP_WING_POINT || type == OPF_SHIP_WING_POINT_OR_NONE) {
						if (find_matching_waypoint(CTEXT(node)) == NULL){
...
int query_operator_argument_type(int op, int argnum)
{
	...
	switch (op) {
		...
		case OP_DISTANCE:
			return OPF_SHIP_WING_TEAM_POINT;
...
*/
	...
	{ OP_ALTER_SHIP_FLAG, "alter-ship-flag\r\n"
		...
		"\tRest:\t (optional) Name of ships or wings. If not supplied, will work on all ships in the mission" },
