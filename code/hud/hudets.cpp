...
//	new_energy = fl_frametime * sinfo_p->power_output;
	...
	if (ship_p->effective_power_drain != ship_p->power_drain) {
		if (ship_p->effective_power_drain < ship_p->power_drain) {
			// Negative power drain means power boost. (Flying into a Descent style energy center?)
...
// _argv[-1] - reduced from 3. give engines low priority.
...
void ai_manage_ets(object* obj)
{
	...
	// _argv[-1] - changed this so that ETS adjustment is allowed if the ship has more than one system to power.
	if ( (ship_p->ship_initial_shield_strength ? 1 : 0) + (ship_info_p->max_speed ? 1 : 0) + (ship_info_p->max_weapon_reserve ? 1 : 0) < 2)
		return;
	...
	if (!no_shields) {
		if ( shield_left_percent < SHIELDS_EMERG_LEVEL_PERCENT ) {
			...
			if (ship_p->target_weapon_energy_delta == 0.0f && shield_left_percent > SHIELDS_MIN_LEVEL_PERCENT)	// dampen ai enthusiasm for sucking energy to weapons
