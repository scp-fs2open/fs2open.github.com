...
// NOTE: Certain goals (Form On Wing, Rearm, Chase Weapon, Fly To Ship) aren't listed here.  This may or may not be
// intentional, but if they are added in the future, it will be necessary to verify correct functionality in the various
// FRED dialog functions.
ai_goal_list Ai_goal_list[] = {{"Waypoints", AI_GOAL_WAYPOINTS, 0},
...
bool Editor::loadMission(const std::string& mission_name, int flags)
{
	...
	size_t pack_offset;
	if (cf_find_file_location(filepath.c_str(), CF_TYPE_MISSIONS, sizeof(fullpath) - 1, fullpath, nullptr,
	                          &pack_offset)) {
	...
	if (parse_main(filepath.c_str(), flags)) {
		if (flags & MPF_IMPORT_FSM) {
			...
			_lastActiveViewport->dialogProvider->showButtonDialog(DialogType::Error, "Load Error", msg,
			                                                      {DialogButton::Ok});
		} else {
			SCP_string msg;
	...
	if ((Num_unknown_ship_classes > 0) || (Num_unknown_weapon_classes > 0) || (Num_unknown_loadout_classes > 0)) {
		if (flags & MPF_IMPORT_FSM) {
			SCP_string msg;
			sprintf(msg,
			        "Fred encountered unknown ship/weapon classes when importing \"%s\" (path \"%s\"). You will have "
			        "to manually edit the converted mission to correct this.",
			        The_mission.name, filepath.c_str());

			_lastActiveViewport->dialogProvider->showButtonDialog(DialogType::Warning, "Unknown Ship classes", msg,
			                                                      {DialogButton::Ok});
		} else {
			_lastActiveViewport->dialogProvider->showButtonDialog(DialogType::Warning, "Unknown Ship classes",
			                                                      "Fred encountered unknown ship/weapon classes when "
			                                                      "parsing the mission file. This may be due to "
			                                                      "mission disk data you do not have.",
			                                                      {DialogButton::Ok});
		}
	}
	...
	for (i = 0; i < Num_teams; i++) {
		...
		for (j = 0; j < Team_data[i].num_weapon_choices; j++) {
			// The amount used in wings is always set by a static loadout entry so skip any that were set by Sexp
			// variables
...
int Editor::getNumMarked() { return numMarked; }
...
int Editor::common_object_delete(int obj)
{
	...
	if (type == OBJ_START) {
		...
		if (Player_starts < 2) { // player 1 start
			_lastActiveViewport->dialogProvider->showButtonDialog(
			    DialogType::Error, "Error", "Must have at least 1 player starting point!", {DialogButton::Ok});
...
void Editor::setActiveViewport(EditorViewport* viewport) { _lastActiveViewport = viewport; }
...
int Editor::reference_handler(const char* name, int type, int obj)
{
	...
	if (r) {
		...
		default: // very bad.  Someone added an sexp somewhere and didn't change this.
			Warning(LOCATION,
			        "\"%s\" referenced by an unknown sexp source!  "
			        "Run for the hills and let Hoffoss know right now!",
			        name);
		...
		sprintf(msg,
		        ...
		        "(click Cancel to go to the reference)",
		        type_name, text);
...
{
	auto r = _lastActiveViewport->dialogProvider->showButtonDialog(
	    DialogType::Warning, "Warning", msg, {DialogButton::Yes, DialogButton::No, DialogButton::Cancel});
...
bool Editor::global_error_check()
{
	...
	if (!z) {
		_lastActiveViewport->dialogProvider->showButtonDialog(
		    DialogType::Information, "Woohoo!", "No errors were detected in this mission", {DialogButton::Ok});
	}
			...
			if (wing >= 0) {                         // ship is part of a wing, so check this
				...
				if (strlen(Wings[wing].wing_squad_filename) > 0) //-V805
				{
					if (The_mission.game_type & MISSION_TYPE_MULTI) {
						...
						if (ptr->type == OBJ_START) {
							if (error("A squad logo was assigned to the player's wing.  The player's squad logo will "
							          "be displayed instead of the wing squad logo on ships in this wing.")) {
								return 1;
							}
						}
					}
	...
	for (i = 0; i < MAX_WINGS; i++) {
		...
		for (j = 0; j < Wings[i].wave_count; j++) {
			...
			if (j == 0) {
				...
				if (error("%s and %s will accept different orders. All ships in a wing must accept the same Player "
				          "Orders.",
...
int Editor::error(const char* msg, ...)
{
	...
	g_err = 1;
	if (_lastActiveViewport->dialogProvider->showButtonDialog(
	        DialogType::Error, "Error", buf, {DialogButton::Ok, DialogButton::Cancel}) == DialogButton::Ok) {
		return 0;
	}
...
int Editor::internal_error(const char* msg, ...)
{
	...
	sprintf_safe(buf2,
	             ...
	             "know about this so he can fix it.  Click cancel to debug.",
	             buf);

	if (_lastActiveViewport->dialogProvider->showButtonDialog(DialogType::Error, "Internal Error", buf2,
...
int Editor::fred_check_sexp(int sexp, int type, const char* msg, ...)
{
	...
	sprintf(error_buf, "Error in %s: %s\n\nIn sexpression: %s\n\n(Error appears to be: %s)", buf.c_str(),
	        sexp_error_message(z), sexp_buf.c_str(), Sexp_nodes[faulty_node].text);
