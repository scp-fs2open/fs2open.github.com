...
void load_gauge(int gauge, int base_w = -1, int base_h = -1, int ship_idx = -1);

#define HUD_OBJECT_CUSTOM				0
void load_gauge_custom(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_MESSAGES				1
void load_gauge_messages(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_TRAINING_MESSAGES		2
void load_gauge_training_messages(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_SUPPORT				3
void load_gauge_support(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_DAMAGE				4
void load_gauge_damage(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_WINGMAN_STATUS		5
void load_gauge_wingman_status(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_AUTO_SPEED			6
void load_gauge_auto_speed(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_AUTO_TARGET			7
void load_gauge_auto_target(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_CMEASURES				8
void load_gauge_countermeasures(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_TALKING_HEAD			9
void load_gauge_talking_head(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_DIRECTIVES			10
void load_gauge_directives(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_WEAPONS				11
void load_gauge_weapons(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_OBJ_NOTIFY			12
void load_gauge_objective_notify(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_SQUAD_MSG				13
void load_gauge_squad_message(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_LAG					14
void load_gauge_lag(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_MINI_SHIELD			15
void load_gauge_mini_shields(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_PLAYER_SHIELD			16
void load_gauge_player_shields(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_TARGET_SHIELD			17
void load_gauge_target_shields(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_ESCORT				18
void load_gauge_escort_view(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_MISSION_TIME			19
void load_gauge_mission_time(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_ETS_WEAPONS			20
void load_gauge_ets_weapons(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_ETS_SHIELDS			21
void load_gauge_ets_shields(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_ETS_ENGINES			22
void load_gauge_ets_engines(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_TARGET_MONITOR		23
void load_gauge_target_monitor(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_EXTRA_TARGET_DATA		24
void load_gauge_extra_target_data(int base_w, int base_h, int ship_idx = -1);

#define HUD_OBJECT_RADAR_STD				25
void load_gauge_radar_std(int base_w, int base_h, int ship_idx = -1);
