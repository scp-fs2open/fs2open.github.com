...
static int joy_haptic_handling_scaler = 0;
...
static bool joy_haptic_create_effects();
static void joy_haptic_start_effect(haptic_effect_t *eff, const char *name);
static void joy_haptic_update_effect(haptic_effect_t *eff, const char *name);
static void joy_haptic_stop_effect(haptic_effect_t *eff, const char *name);
...
static void print_haptic_support() {
	...
	auto supported = SDL_GetHapticFeatures(haptic);
...
bool joy_haptic_init()
{
	...
	auto haptics = SDL_GetHaptics(nullptr);

	if ( !haptics ) {
		mprintf(("    <none>\n"));
	} else {
		for (int i = 0; haptics[i]; ++i) {
	...
	haptic = SDL_OpenHapticFromJoystick(Joy->getJoystick());
...
static bool joy_haptic_can_play()
{
...
void joy_haptic_set_gain(int gain)
{
	...
	if (SDL_GetHapticFeatures(haptic) & SDL_HAPTIC_GAIN) {
		SDL_SetHapticGain(haptic, gain);
	} else {
		Gain = gain / 100.0f;
		Gain_adjust = (gain < 100);

		// force update spring effect now since it only changes at mission start
		joy_haptic_update_effect(&pSpring, "Spring");
	}
}

static void haptic_adjust_effect_strength(SDL_HapticEffect *eff)
{
	if ( !Gain_adjust ) {
		return;
	}
	...
	switch (eff->type) {
		...
		case SDL_HAPTIC_SAWTOOTHDOWN:
			eff->periodic.magnitude = Sint16(eff->periodic.magnitude * Gain);
			break;

		case SDL_HAPTIC_CONSTANT:
			eff->constant.level = Sint16(eff->constant.level * Gain);
			break;

		// special case for spring
		case SDL_HAPTIC_SPRING: {
			auto val = Uint16(0x7FFF * Gain);

			for (size_t i = 0; i < SDL_arraysize(eff->condition.right_sat); i++) {
				eff->condition.right_sat[i] = val;
				eff->condition.left_sat[i] = val;
			}
...
static bool haptic_create_effect(haptic_effect_t *effect)
{
	// manually adjust effect strength if necessary
	haptic_adjust_effect_strength(&effect->eff);

	effect->id = SDL_CreateHapticEffect(haptic, &effect->eff);
...
static bool joy_haptic_create_effects()
{
	...
	if (supported & SDL_HAPTIC_SPRING) {
		...
		if ( !haptic_create_effect(&pSpring) ) {
	...
	if (supported & SDL_HAPTIC_SAWTOOTHDOWN) {
		...
		if ( !haptic_create_effect(&pShootEffect) ) {
	...
	if (supported & SDL_HAPTIC_CONSTANT) {
		...
		if ( !haptic_create_effect(&pSecShootEffect) ) {
	...
	if (supported & SDL_HAPTIC_SINE) {
		...
		if ( !haptic_create_effect(&pAfterburn1) ) {
		...
		if ( !haptic_create_effect(&pAfterburn2) ) {
	...
	if (supported & SDL_HAPTIC_CONSTANT) {
		...
		if ( !haptic_create_effect(&pHitEffect1) ) {
	...
	if (supported & SDL_HAPTIC_SINE) {
		...
		if ( !haptic_create_effect(&pHitEffect2) ) {
...
static void joy_haptic_update_effect(haptic_effect_t *eff, const char* /* name */)
{
	...
	// manually adjust effect strength if necessary
	haptic_adjust_effect_strength(&eff->eff);
...
void joy_haptic_stop_effects()
{
	...
	joy_haptic_stop_effect(&pHitEffect1, "HitEffect1");
	joy_haptic_stop_effect(&pHitEffect2, "HitEffect2");
	joy_haptic_stop_effect(&pAfterburn1, "Afterburn1");
	joy_haptic_stop_effect(&pAfterburn2, "Afterburn2");
	joy_haptic_stop_effect(&pShootEffect, "ShootEffect");
	joy_haptic_stop_effect(&pSecShootEffect, "SecShootEffect");
	joy_haptic_stop_effect(&pDock, "Dock");
}
...
void joy_haptic_mission_init(vec3d *v)
{
	...
	joy_haptic_handling_scaler = (int) ((vm_vec_mag(v) + 1.3f) * 5.0f);

	joy_haptic_adjust_handling(0);
	joy_haptic_start_effect(&pSpring, "Spring");
	...
	joy_haptic_update_effect(&pAfterburn1, "Afterburn1 (init)");
	...
	joy_haptic_update_effect(&pAfterburn2, "Afterburn2 (init)");
	...
	joy_haptic_update_effect(&pShootEffect, "ShootEffect (init)");
}
...
void joy_haptic_play_dir_effect(float x, float y)
{
	...
	if (joy_ff_hit_effect_enabled()) {
		...
		joy_haptic_update_effect(&pHitEffect1, "HitEffect1");
		...
		joy_haptic_update_effect(&pHitEffect2, "HitEffect2");
	}

	joy_haptic_start_effect(&pHitEffect1, "HitEffect1");
	joy_haptic_start_effect(&pHitEffect2, "HitEffect2");
}
...
void joy_haptic_play_primary_shoot(int gain)
{
	...
	if (gain != primary_ff_level) {
		...
		joy_haptic_update_effect(&pShootEffect, "ShootEffect");

		primary_ff_level = gain;
	}

	joy_haptic_start_effect(&pShootEffect, "ShootEffect");
}
...
void joy_haptic_play_secondary_shoot(int gain)
{
	...
	if (gain != secondary_ff_level) {
		...
		joy_haptic_update_effect(&pSecShootEffect, "SecShootEffect");

		secondary_ff_level = gain;
//		nprintf(("Joystick", "FF: Secondary force = 0x%04x\n", pSecShootEffect.eff.constant.level));
	}

	joy_haptic_start_effect(&pSecShootEffect, "SecShootEffect");
}
...
void joy_haptic_adjust_handling(int speed)
{
	...
	v = speed * joy_haptic_handling_scaler * 2 / 3;
//	v += joy_haptic_handling_scaler * joy_haptic_handling_scaler * 6 / 7 + 250;
	v += joy_haptic_handling_scaler * 45 - 500;
...
void joy_haptic_docked()
{
	...
	joy_haptic_update_effect(&pDock, "Dock");
	joy_haptic_start_effect(&pDock, "Dock");
}
...
void joy_haptic_play_reload_effect()
{
	...
	joy_haptic_update_effect(&pDock, "Dock (Reload)");
	joy_haptic_start_effect(&pDock, "Dock (Reload)");
}
...
void joy_haptic_afterburn_on()
{
	...
	joy_haptic_update_effect(&pAfterburn1, "Afterburn1");

	pAfterburn2.eff.periodic.length = SDL_HAPTIC_INFINITY;
	pAfterburn2.eff.periodic.magnitude = 0x1999;

	joy_haptic_update_effect(&pAfterburn2, "Afterburn2");

	joy_haptic_start_effect(&pAfterburn1, "Afterburn1");
	joy_haptic_start_effect(&pAfterburn2, "Afterburn2");
...
void joy_haptic_explode()
{
	...
	if (pShootEffect.loaded) {
		...
		joy_haptic_stop_effect(&pShootEffect, "ShootEffect (Explode)");
		...
		joy_haptic_update_effect(&pShootEffect, "ShootEffect (Explode)");

		joy_haptic_start_effect(&pShootEffect, "ShootEffect (Explode)");
	}
}
...
void joy_haptic_fly_by(int mag)
{
	...
	joy_haptic_update_effect(&pAfterburn1, "Afterburn1 (Fly by)");

	pAfterburn2.eff.periodic.length = (6000 * mag + 400000) / 1000;
	pAfterburn2.eff.periodic.magnitude = (Sint16) fl2i(0x7FFF * (gain / 10000.0f));

	joy_haptic_update_effect(&pAfterburn2, "Afterburn2 (Fly by)");

	joy_haptic_start_effect(&pAfterburn1, "Afterburn1 (Fly by)");
	joy_haptic_start_effect(&pAfterburn2, "Afterburn2 (Fly by)");
}
...
void joy_haptic_deathroll()
{
	...
	joy_haptic_update_effect(&pAfterburn1, "Afterburn1 (Death Roll)");
	...
	joy_haptic_update_effect(&pAfterburn2, "Afterburn2 (Death Roll)");

	joy_haptic_start_effect(&pAfterburn1, "Afterburn1 (Death Roll)");
	joy_haptic_start_effect(&pAfterburn2, "Afterburn2 (Death Roll)");
}
