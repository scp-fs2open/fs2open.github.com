...
#ifdef _WIN32
extern DWORD win32_process(DWORD lparam);

int opengl_init_pixel_format_WIN()
{
	HWND hwnd;
	HDC hdc;
	HGLRC hglrc;
	...
	const char *WGL_extension_string = NULL;

	const char *(WINAPI *wglGetExtensionsStringARB)(HDC) = NULL;
	BOOL (WINAPI *wglChoosePixelFormatARB)(HDC, const int *, const FLOAT *, UINT, int *, UINT *) = NULL;
	BOOL (WINAPI *wglGetPixelFormatAttribivARB)(HDC, int, int, UINT, const int *, int *) = NULL;

	// --- init WGL stuff ------------------------------------------------------
	hwnd = CreateWindow(Osreg_app_name, Osreg_title, WS_POPUP | WS_DISABLED, 0, 0, 10, 10, NULL, NULL, hInst, NULL);

	win32_process(0);

	hdc = GetDC(hwnd);

	PixelFormat = ChoosePixelFormat(hdc, &GL_pfd);
printf("PixelFormat: %i\n", PixelFormat);
	SetPixelFormat(hdc, PixelFormat, &GL_pfd);

	hglrc = wglCreateContext(hdc);

	if ( !hglrc ) {
		printf("AAAAAAAAHSAFHDSDHSDHFSDF\n");
		return 0;
	}
printf("here!!!!\n");
	...
	if (wglGetExtensionsStringARB != NULL) {
		WGL_extension_string = wglGetExtensionsStringARB(hdc);
	}

	if ( (WGL_extension_string != NULL) && strstr(WGL_extension_string, "WGL_ARB_pixel_format") ) {
		wglChoosePixelFormatARB = (BOOL (WINAPI *)(HDC, const int *, const FLOAT *, UINT, int *, UINT *)) wglGetProcAddress("wglChoosePixelFormatARB");
		wglGetPixelFormatAttribivARB = (BOOL (WINAPI *)(HDC, int, int, UINT, const int *, int *)) wglGetProcAddress("wglGetPixelFormatAttribivARB");
	}
mprintf(("got what we need!\n"));
	wglMakeCurrent(NULL, NULL);
...
mprintf(("done with wgl stuff!\n"));
	...
	ADD_ATTRIB( WGL_PIXEL_TYPE_ARB, WGL_TYPE_RGBA_ARB );

	ADD_ATTRIB( WGL_RED_BITS_ARB, Gr_red.bits );
	ADD_ATTRIB( WGL_GREEN_BITS_ARB, Gr_green.bits );
	ADD_ATTRIB( WGL_BLUE_BITS_ARB, Gr_blue.bits );

	ADD_ATTRIB( WGL_DOUBLE_BUFFER_ARB, GL_TRUE );

	ADD_ATTRIB( WGL_COLOR_BITS_ARB, (gr_screen.bits_per_pixel == 32) ? 32 : 16 );
	ADD_ATTRIB( WGL_ALPHA_BITS_ARB, (gr_screen.bits_per_pixel == 32) ? 8 : 0 );
	ADD_ATTRIB( WGL_DEPTH_BITS_ARB, (gr_screen.bits_per_pixel == 32) ? 24 : 16 );

//	ADD_ATTRIB( WGL_SAMPLE_BUFFERS_ARB, 1 );
//	ADD_ATTRIB( WGL_SAMPLES_ARB, 2 );



	for (i = 0; i < 2; i++) {
		if (wglChoosePixelFormatARB != NULL) {
			wglChoosePixelFormatARB(GL_device_context, iAttribs, fAttribs, 1, &PixelFormat, &num_matching);
	mprintf(("used wglChoosePixelFormatARB!\n"));
		}
		...
		if ( !SetPixelFormat(GL_device_context, PixelFormat, &GL_pfd) ) {
			// try to recover by resetting the window and getting the pixel format one more time
			if (i == 0) {
	mprintf(("attempting to recreate window!\n"));
				extern void win32_create_window(int, int);
				win32_create_window( gr_screen.max_w, gr_screen.max_h );

				continue;
			}

			MessageBox((HWND)os_get_window(), "Unable to set pixel format for OpenGL W32!", "error", MB_ICONERROR | MB_OK);
			return 0;
		}
...
int opengl_init_display_device()
{
	...
	if ( (PixelFormat = opengl_init_pixel_format_WIN()) == 0 ) {
		Int3();
	}
