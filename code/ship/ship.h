...
typedef struct ship_weapon {
	...
	Ship::weapon_flags flags;								// see SW_FLAG_* defines above
...
public:
	...
	Ship::subsystem_flags flags;						// Goober5000
	...
	Ship::subsys_sound_flags subsys_snd_flags;
...
typedef struct ship_type_info {
	...
	//Messaging?
	Ship::type_info_msg message_bools;

	//HUD
	Ship::type_info_hud hud_bools;

	//Ship
	Ship::type_info_ship ship_bools;	//For lack of better term
	float debris_max_speed;

	//Weapons
	Ship::type_info_weapon weapon_bools;
	float ff_multiplier;
...
Ship::info_flags ship_get_SIF(ship *shipp);
Ship::info_flags ship_get_SIF(int sh);
...
inline bool should_be_ignored(ship* shipp) {
	return shipp->flags[Ship::Ship_Flags::Exploded] || shipp->flags[Ship::Ship_Flags::Depart_warp] || shipp->flags[Ship::Ship_Flags::Dying] || shipp->flags[Ship::Ship_Flags::Arriving_stage_1] || shipp->flags[Ship::Ship_Flags::Hidden_from_sensors];
}
