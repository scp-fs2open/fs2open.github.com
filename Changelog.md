...
- Re-Add `-joy-info` cmdline flag, for slightly quicker Joystick GUID retrieval
- Add `-controlconfig_tbl` cmdline flag to print out a template controlconfigdefaults.tbl
</details>
...
- Command-line parameter `-fov_cockpit` which allows setting a FoV for the cockpit model separately from the viewport FoV. This allows using custom FoVs without distorting the cockpit. 
...
- `cancel-future-waves`, for canceling wings that still had waves queued.
- Added optional flag to `show-subtitle-text` and `show-subtitle-image` to not change subtitle aspect ratio
...
- Add methods to compare timestamps, calculate deltas from an existing timestamp and a number of miliseconds, and to calculate deltas between timestamps
...
- Restore original Error Checking behavior where function would exit after one check (for consistency)
...
- Cached subsystem indexing, speeding up subsystem lookups, which runs for multiple ships, multiple times per frame.
</details>
...
- Make `set-camera-facing` use default identity orientation when `$Use host orientation` flag is active and no host is present. 
- When a hotkey text replacement string refers to an unbound control, use the bind name instead of "none"
...
- Update and correctly load envmaps in F3 Lab.
...
- Ignore HUD brackets with size of zero or less (instead of just crashing)
...
- Check for valid index before accessing `Weapon_info`, preventing CTDs under certain conditions.
...
- Fix a crash in Ship Creation code if a ship's name was exactly the maximum allowed length
