...
TEST_F(SplitStrTest, split_str_once_typical)
{
	...
	// no split - a string that contains spaces but still fits should not be split
	{
		auto [len, pos, forced] = split_str_once("abc defg", 300);
		ASSERT_EQ(len, 8);
		ASSERT_EQ(pos, 0);
		ASSERT_EQ(forced, false);
	}

	// no split - as above, but trailing grayspace is trimmed
	{
		auto [len, pos, forced] = split_str_once("abc defg   ", 300);
		ASSERT_EQ(len, 8);
		ASSERT_EQ(pos, 0);
...
TEST_F(SplitStrTest, split_str_once_advanced)
{
	...
	{
		// a leading space is a natural break like any other, so it's used in preference to a forced split (producing an empty first line)
		auto [len, pos, forced] = split_str_once(" abcdefghijklmnopqrstuvwxyz", 100);	// wide enough for the space, but not wide enough for the whole string
		ASSERT_EQ(len, 0);
		ASSERT_EQ(pos, 1);
		ASSERT_EQ(forced, false);
	}
