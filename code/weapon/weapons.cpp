...
int parse_weapon(int subtype, bool replace, const char *filename)
{
  ...
  parse_optional_float_into("@Laser Min Pixel Size:", &wip->laser_min_pixel_size);
	...
	} else if (first_time && (wip->wi_flags[Weapon::Info_Flags::Turret_Interceptable] ||
								 wip->wi_flags[Weapon::Info_Flags::Fighter_Interceptable] || wip->wi_flags[Weapon::Info_Flags::Mine])) {
