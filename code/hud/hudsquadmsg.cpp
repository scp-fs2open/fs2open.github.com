...
int hud_squadmsg_count_ships(int add_to_menu)
{
	...
	{	
		...
		if ( Ship_info[shipp->ship_info_index].class_type < 0 || !(Ship_types[Ship_info[shipp->ship_info_index].class_type].ai_bools[Ship::Type_Info_AI::Accept_player_orders]) )
			continue;
...
int hud_squadmsg_wing_valid(wing *wingp)
{
	...
	if ( wingp->flags[Ship::Wing_Flags::Gone] || wingp->flags[Ship::Wing_Flags::Departing] )
		return 0;
...
void hud_squadmsg_type_select( )
{
	...
	if (Ai_info[Ships[Player_obj->instance].ai_index].ai_flags[AI::AI_Flags::Awaiting_repair] || Ai_info[Ships[Player_obj->instance].ai_index].ai_flags[AI::AI_Flags::Being_repaired]) {
...
void hud_squadmsg_call_reinforcement(int reinforcement_num, int player_num)
{
	...
	for (i = 0; i < Num_wings; i++ ) {
		if ( !stricmp(rp->name, Wings[i].name) ) {
			...
			Wings[i].flags.unset(Ship::Wing_Flags::Reinforcement);
	...
	if ( i == Num_wings ) {
		...
		if ( p_objp ) {
			...
			p_objp->flags.unset(Mission::Parse_Object_Flags::SF_Reinforcement);
