...
#define ADD_LONG(d) do { Assert((packet_size + sizeof(d)) < MAX_PACKET_SIZE); long swap = INTEL_INT(d); memcpy(data+packet_size, &swap, sizeof(d) ); packet_size += sizeof(d); } while (0)
#define ADD_ULONG(d) do { Assert((packet_size + sizeof(d)) < MAX_PACKET_SIZE); ulong swap = INTEL_INT(d); memcpy(data+packet_size, &swap, sizeof(d) ); packet_size += sizeof(d); } while (0)
...
#define GET_LONG(d) do { long swap; memcpy(&swap, data+offset, sizeof(d) ); d = INTEL_LONG(swap); offset += sizeof(d); } while(0)
