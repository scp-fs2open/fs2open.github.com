...
void main() {
	// Irradiance map is 16x16 — remap fragment position to (-1, 1)
	float xpos = 2.0 * gl_FragCoord.x / 16.0 - 1.0;
	float ypos = 2.0 * gl_FragCoord.y / 16.0 - 1.0;
	vec3 fragDir;
	...
	for (int i = 0; i < 128; ++i)
	{
		// Pre-calculated sampling: fixed directions mirrored onto correct hemisphere.
		// ~50% of envmap texture samples are shared between adjacent fragments,
		// improving texture cache hit rate.
	...
	fragOut0 = linear_to_srgb(PI * fragCol * (1.0 / 128.0));
}
