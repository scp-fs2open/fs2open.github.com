...
SCP_string Cmdline_spew_mission_crcs;
SCP_string Cmdline_spew_table_crcs;
...
int cmdline_parm::get_int()
{
	...
	try {
		...
		Warning(LOCATION, "Failed to convert '%s' to a float for parameter '%s'!", args.c_str(), name);
		return 0;
	}
}
...
static void stuff_mod_option(cmdline_parm* param, json_t* el)
{
	...
	json_array_foreach(el, i, modObj)
	{
...
static bool parse_json_flags(json_t* rootEl)
{
	...
	json_object_foreach(flagsEl, flagName, flagValue)
	{
...
static bool SetCmdlineParams(flag_input_type inputType);

static bool SetCmdlineParamsFromStdin(const SCP_string& format)
{
	if (format != "json_v1") {
		// Invalid format!
	...
	return SetCmdlineParams(flag_input_type::Stdin_Json_V1);
}
...
// Sets externed variables used for communication cmdline information
{
	...
	if (inputType == flag_input_type::Flags && stdin_flags.found()) {
		return SetCmdlineParamsFromStdin(stdin_flags.str());
	}
	...
	if ( missioncrcspew_arg.found() ) {
		...
		if (Cmdline_spew_mission_crcs.back() == ' ') {
			Cmdline_spew_mission_crcs.resize(Cmdline_spew_mission_crcs.size() - 1);
		}
 	}
	...
	if ( tablecrcspew_arg.found() ) {
		...
		// strip off blank space at end if it's there
		if (Cmdline_spew_table_crcs.back() == ' ') {
			Cmdline_spew_table_crcs.resize(Cmdline_spew_table_crcs.size() - 1);
		}
 	}
	...
	if (inputType == flag_input_type::Flags && mod_arg.found()) {
		...
		Cmdline_mod = cfile::getModRootsFromModCmdline(whee, modCmdline.c_str());
	}
...
//	mprintf(("I got to parse_cmdline()!!\n"));
	...
	return SetCmdlineParams(flag_input_type::Flags);
}
