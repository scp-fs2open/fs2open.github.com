...
/**
 * @brief Shader variant definition — maps a (type, flag) to a #define string
 *
 * Used by both OpenGL (GL_shader_variants[]) and Vulkan to generate the
 * preprocessor header for compile-time shader specialization.
 */
...
/**
 * @brief Shader type definition - maps shader type to GLSL filenames
 */
struct VulkanShaderTypeInfo {
	shader_type type;
	const char* vertexFile;      // Vertex shader filename (without extension)
	const char* fragmentFile;    // Fragment shader filename (without extension)
...
private:
	 ...
	 */
	static const VulkanShaderTypeInfo* getShaderTypeInfo(shader_type type);
