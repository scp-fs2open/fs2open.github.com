...
TEST_F(SplitStrTest, split_str_once_advanced)
{
	...
	// whole string fits but contains internal whitespace - must not split
	{
		auto [len, pos, forced] = split_str_once("abcd efgh", 300);
		ASSERT_EQ(len, 9);
		ASSERT_EQ(pos, 0);
		ASSERT_EQ(forced, false);
	}
	{
		auto [len, pos, forced] = split_str_once("fighter cover pronto, we'll never reach Vega!", 1000);
		ASSERT_EQ(len, 45);
		ASSERT_EQ(pos, 0);
		ASSERT_EQ(forced, false);
	}
