...
void do_subobj_destroyed_stuff( ship *ship_p, ship_subsys *subsys, vec3d* hitpos, bool no_explosion )
{
	...
	if(subsys->subsys_snd_flags[Ship::Subsys_Sound_Flags::Alive])
	{
		...
		subsys->subsys_snd_flags.unset(Ship::Subsys_Sound_Flags::Alive);
	}
	if (subsys->subsys_snd_flags[Ship::Subsys_Sound_Flags::Turret_Rotation])
	{
		...
		subsys->subsys_snd_flags.unset(Ship::Subsys_Sound_Flags::Turret_Rotation);
	}
	if (subsys->subsys_snd_flags[Ship::Subsys_Sound_Flags::Rotate])
	{
		...
		subsys->subsys_snd_flags.unset(Ship::Subsys_Sound_Flags::Rotate);
	}
	if ((subsys->system_info->dead_snd != -1) && !(subsys->subsys_snd_flags[Ship::Subsys_Sound_Flags::Dead]))
	{
		...
		subsys->subsys_snd_flags.unset(Ship::Subsys_Sound_Flags::Dead);
	}
}
...
void ship_hit_kill(object *ship_objp, object *other_obj, float percent_killed, int self_destruct)
{
	...
	// mwa -- removed 2/25/98 -- why is this here?  ship_objp->flags.unset(Object::Object_Flags::Player_ship);
...
void ship_apply_global_damage(object *ship_objp, object *other_obj, vec3d *force_center, float damage )
{
	...
	if ( force_center )	{
		...
		if (Cmdline_nohtl && (other_obj != NULL) && (other_obj->type == OBJ_SHOCKWAVE) && (is_huge_ship(&Ship_info[Ships[ship_objp->instance].ship_info_index])))
		{
