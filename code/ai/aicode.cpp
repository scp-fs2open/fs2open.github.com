...
void ai_do_objects_repairing_stuff( object *repaired_objp, object *repair_objp, int how )
{
		...
		if ( p_index >= 0 ) {
			...
			if (Game_mode & GM_NORMAL || MULTIPLAYER_MASTER) {
				if ( how == REPAIR_INFO_KILLED ){
					message_send_builtin(MESSAGE_SUPPORT_KILLED, NULL, 0, 0, p_index, p_team);
				} else {
					if ( repair_objp ){
						message_send_builtin(MESSAGE_REPAIR_ABORTED, &Ships[repair_objp->instance], 0, 0, p_index, p_team);
					}
				}
			}
		}
	...
	case REPAIR_INFO_COMPLETE:
		...
		if ( p_index >= 0 ) {
			...
			if (Game_mode & GM_NORMAL || MULTIPLAYER_MASTER) {
				message_send_builtin(MESSAGE_REPAIR_DONE, &Ships[repair_objp->instance], 0, 0, p_index, p_team);
			}
		}
		stamp = timestamp((int) ((30 + 10*frand()) * 1000));
		break;
...
void process_friendly_hit_message( int message, object *objp )
{
	...
	if ( index >= 0 ) 
	{
		message_send_builtin(message, &Ships[index], 0, 0, -1, -1);
	} else {
		message_send_builtin(message, NULL, 0, 0, -1, -1);
	}
}
		...
		if (obj_team(objp_hitter) == obj_team(objp_hit)) {
			...
			if (is_instructor(objp_hit)) {
				...
				if (pp->friendly_damage > FRIENDLY_DAMAGE_THRESHOLD) {
					message_send_builtin(MESSAGE_INSTRUCTOR_ATTACK, NULL, 0, 0, -1, -1);
					...
					message_send_builtin(MESSAGE_INSTRUCTOR_HIT, NULL, 0, 0, -1, -1);
					pp->last_warning_message_time = Missiontime;
				}
