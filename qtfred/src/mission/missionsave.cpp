	...
	if (list_found) {
		...
		for (const auto &container : containers) {
			if (container.is_list()) {
				...
				if (container.type_includes(ContainerType::STRING_DATA)) {
					fout("\n$Data Type: String");
				} else if (container.type_includes(ContainerType::NUMBER_DATA)) {
					fout("\n$Data Type: Number");
				}

				if (container.type_includes(ContainerType::STRICTLY_TYPED_DATA)) {
					fout("\n+Strictly Typed Data");
				}
	...
	if (map_found) {
		...
		for (const auto &container : containers) {
			if (container.is_map()) {
				...
				if (container.type_includes(ContainerType::NUMBER_KEYS)) {
					fout("\n$Key Type: Number");
				} else {
					fout("\n$Key Type: String");
				}

				if (container.type_includes(ContainerType::STRICTLY_TYPED_KEYS)) {
					fout("\n+Strictly Typed Keys");
				}
...
{
	if (container.type_includes(ContainerType::NETWORK)) {
	...
	if (container.type_includes(ContainerType::SAVE_ON_MISSION_CLOSE)) {
		fout("+Save On Mission Close\n");
	} else if (container.type_includes(ContainerType::SAVE_ON_MISSION_PROGRESS)) {
