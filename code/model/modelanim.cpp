...
{	
	...
	if ( !strnicmp(p, "docking", 7) || !strnicmp(p, "\"docking\"", 9) ) {
		mprintf(("The \"docking\" animation type name is deprecated.  Specify \"%s\" instead.", Animation_type_names[TRIGGER_TYPE_DOCKING_STAGE_2]));
		return TRIGGER_TYPE_DOCKING_STAGE_2;
	} else if ( !strnicmp(p, "primary_bank", 12) || !strnicmp(p, "\"primary_bank\"", 14) ) {
		mprintf(( "The \"primary_bank\" animation type name is deprecated.  Specify \"%s\" instead.", Animation_type_names[TRIGGER_TYPE_PRIMARY_BANK]));
		return TRIGGER_TYPE_PRIMARY_BANK;
	} else if ( !strnicmp(p, "secondary_bank", 14) || !strnicmp(p, "\"secondary_bank\"", 16) ) {
		mprintf(("The \"secondary_bank\" animation type name is deprecated.  Specify \"%s\" instead.", Animation_type_names[TRIGGER_TYPE_SECONDARY_BANK]));
		return TRIGGER_TYPE_SECONDARY_BANK;
	} else if ( !strnicmp(p, "door", 4) || !strnicmp(p, "\"door\"", 6) ) {
		mprintf(("The \"door\" animation type name is deprecated.  Specify \"%s\" instead.", Animation_type_names[TRIGGER_TYPE_DOCK_BAY_DOOR]));
		return TRIGGER_TYPE_DOCK_BAY_DOOR;
	} else if ( !strnicmp(p, "turret firing", 13) || !strnicmp(p, "\"turret firing\"", 15) ) {
		mprintf(("The \"turret firing\" animation type name is deprecated.  Specify \"%s\" instead.", Animation_type_names[TRIGGER_TYPE_TURRET_FIRING]));
	...
	for (i = 0; i < MAX_TRIGGER_ANIMATION_TYPES; i++) {
		...
		if ( !strnicmp(p, quoted_name, strlen(quoted_name)) ) {
			mprintf(( "Old usage warning: Please remove quotes from animation type %s.", quoted_name));
			return i;
		}
	}
