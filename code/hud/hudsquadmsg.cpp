...
void hud_squadmsg_repair_rearm( int toggle_state, object *objp)
{
	...
	if ( is_support_allowed(tobj) ) {
		if ( mission_is_repair_scheduled( tobj ) ) {
			message_send_builtin_to_player(MESSAGE_ALREADY_ON_WAY, NULL, 0, 0, multi_player_num, multi_player_team);
		} else {
			robjnum = hud_support_find_closest(OBJ_INDEX(tobj));
			if ( robjnum != -1 ) {
				message_send_builtin_to_player(MESSAGE_ALREADY_ON_WAY, &Ships[Objects[robjnum].instance], 0, 0, multi_player_num, multi_player_team);
			} else {
				...
				if ( robjnum != -1) {
					...
					message_send_builtin_to_player(MESSAGE_ON_WAY, &Ships[robjp->instance], 0, 0, multi_player_num, multi_player_team);

				} else {
					// if we are in this part of the if statement, a support ship has been warped in to
					// service us.  Issue appropriate message
					message_send_builtin_to_player(MESSAGE_REARM_WARP, NULL, 0, 0, multi_player_num, multi_player_team);
				}
...
int hud_squadmsg_send_ship_command( int shipnum, int command, int send_message, int update_history, int player_num )
{
	...
	if (send_message && (!(Ships[shipnum].flags[Ship::Ship_Flags::No_builtin_messages]))) {
		message_send_builtin_to_player(message, &Ships[shipnum], 0, 0, player_num, message_team_filter);
	}
...
int hud_squadmsg_send_wing_command( int wingnum, int command, int send_message, int update_history, int player_num )
{
	...
	if (send_message) {
		...
		if (ship_num != -1) {
			message_send_builtin_to_player(message, &Ships[ship_num], 0, 0, player_num, message_team_filter);
			message_sent = 1;
		}
	}
