...
// Calculates the eye position for this ship in the global (by default) or local reference frame.  Uses the
...
void ship_get_eye(vec3d *eye_pos, matrix *eye_orient, const object *obj, bool do_slew, bool local_reference_frame)
{
	...
	if ((pm->n_view_positions <= 0) || (current_viewpoint < 0) || (current_viewpoint >= pm->n_view_positions)) {
	...
	model_instance_local_to_global_point_orient(eye_pos, eye_orient, &ep.pnt, &eye_local_orient, pm, pmi, ep.parent, local_reference_frame ? nullptr : &obj->orient, local_reference_frame ? nullptr : &obj->pos);
