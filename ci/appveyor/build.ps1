...
if ([System.Convert]::ToBoolean($DeployBuild)) {
	# Release build
	if (![System.Convert]::ToBoolean($DeployConfig)) {
        exit 0
	}
    ...
    7z a "$($PackageName)-builds-Win64.zip" "$env:APPVEYOR_BUILD_FOLDER/../install/*"
    Push-AppveyorArtifact "$($PackageName)-builds-Win64.zip"
} else {
