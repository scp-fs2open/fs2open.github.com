...
bool test_line_of_sight(vec3d* from, vec3d* to, std::unordered_set<const object*>&& excluded_objects, float threshold, bool test_for_shields, bool test_for_hull, float* first_intersect_dist, int* first_intersect_signature) {
	bool collides = false;
			...
			if (model_collide(&hull_check)) {
				...
				if (first_intersect_dist == nullptr) {
					...
					if (!collides || *first_intersect_dist > dist) {
						...
						*first_intersect_signature = objp->signature;
					}
					collides = true;
				}
			}
