...
#ifndef NDEBUG
vector tvec = objp->orient.vec.fvec;
vector	vel_in_copy;
...
void evaluate_object_as_nearest_objnum(eval_nearest_objnum *eno)
{
	...
	if ((attacking_subsystem != NULL) || !(eno->trial_objp->flags & OF_PROTECTED)) {
		if ( OBJ_INDEX(eno->trial_objp) != eno->objnum ) {
			...
			// Goober5000: don't evaluate the exceptions
			if (eno->except_flag)
			{
				int i, test_num;
				ai_goal *aigp = &aip->goals[aip->active_goal];

				for (i = 0; i < aigp->num_special_objects; i++)
				{
					// a wing?
					...
					{
						test_num = eno->trial_objp->instance;
					}

					// if this is an exception, don't evaluate it
					if (aigp->special_object_num[i] == test_num)
						return;
				}
			}
...
int get_nearest_objnum(int objnum, int enemy_team_mask, int enemy_wing, float range, int max_attackers, int except)
{
	...
	if ((eno.nearest_objnum == -1) && (enemy_wing != -1)) {
		return get_nearest_objnum(objnum, enemy_team_mask, -1, range, max_attackers, except);
	}
...
//	Don't attack a ship that already has at least max_attackers attacking it.
int find_enemy(int objnum, float range, int max_attackers, int except)
{
	int	enemy_team_mask;
	...
	if (timestamp_elapsed(aip->choose_enemy_timestamp)) {
		...
		return get_nearest_objnum(objnum, enemy_team_mask, aip->enemy_wing, range, max_attackers, except);
	} else {
		aip->target_objnum = -1;
		aip->target_signature = -1;
		return -1;
...
void ai_attack_object(object *attacker, object *attacked, int priority, ship_subsys *ssp, int except)
{
	ai_info	*aip;
...
//		return;
	...
	if (attacked == NULL) {
		...
		// nebula safe
		set_target_objnum(aip, find_enemy(attacker-Objects, 99999.9f, 4, except));
	} else {
...
void get_behind_ship(ai_info *aip, ship_info *sip, float dist_to_enemy)
{
	...
	// Goober5000: removed this... why was it added?
	// slide_face_ship();//added this -Bobboau
}
...
//	Return distance.
float dock_move_towards_point(object *objp, vector *start, vector *finish, vector *submodel_pos, float submodel_radius, float submodel_omega, int dock_mode, float speed_scale, float other_obj_speed = 0.0f)
{
...
float dock_orient_and_approach(object *docker_objp, int docker_index, object *dockee_objp, int dockee_index, int dock_mode, vector *docker_point_param, vector *dockee_point_param, float *rotating_submodel_tangential_velocity)
{
	ship_info	*sip0, *sip1;
	polymodel	*pm0, *pm1;
	ai_info		*aip;
