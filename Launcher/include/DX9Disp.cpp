// DX9Disp.cpp : implementation file
//
#include "stdafx.h"
...
// Declared here to avoid having to include DX9 files in the header file
IDirect3D9* d3d_interface = NULL;
...
CDX9Disp::CDX9Disp(CWnd* pParent /*=NULL*/)
	: CDialog(CDX9Disp::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDX9Disp)
	//}}AFX_DATA_INIT

	m_dx_initialised_ok = false;
	m_allow_only_standard_modes = true;
}


void CDX9Disp::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDX9Disp)
...
BEGIN_MESSAGE_MAP(CDX9Disp, CDialog)
	//{{AFX_MSG_MAP(CDX9Disp)
...
/////////////////////////////////////////////////////////////////////////////
// CDX9Disp message handlers

void CDX9Disp::OnSelchangeAdapterList() 
{
	UpdateResList();
}

void CDX9Disp::OnSelchangeResList() 
{
	UpdateAntialiasList();
}

void CDX9Disp::OnSelchangeAntialiasList() 
{
}

BOOL CDX9Disp::OnInitDialog() 
{
	...
	// User does not have DX9 installed disable all functionality
...
void CDX9Disp::UpdateAdapterList(int select)
{
#ifdef DX9_ENABLED
 ...
 */
int d3d9_get_mode_bit(D3DFORMAT type)
{
	switch(type)
	{
	...
	unsigned int requested_width, unsigned int requested_height, int requested_cdepth)
{
	...
	int num_modes		  = d3d_interface->GetAdapterModeCount(selected_adapter, D3DFMT_X8R8G8B8);
	...
	{
		D3DDISPLAYMODE mode;
		if(FAILED(d3d_interface->EnumAdapterModes(selected_adapter, D3DFMT_X8R8G8B8, i, &mode)))
		{
		...
		bool non_standard_match = 
			...
			d3d9_get_mode_bit(mode.Format) == requested_cdepth);
		...
		while(i > 0)
		{
			...
			if(FAILED(d3d_interface->EnumAdapterModes(selected_adapter, D3DFMT_X8R8G8B8, i-1, &next_mode)))
			{
			...
			if( mode.Width == next_mode.Width && 
				...
				d3d9_get_mode_bit(mode.Format) == d3d9_get_mode_bit(next_mode.Format)) 
			{
		...
		char mode_string[20];
		sprintf(mode_string, "%dx%dx%d", mode.Width, mode.Height, d3d9_get_mode_bit(mode.Format));
		...
		if( requested_width  == mode.Width &&
			...
			requested_cdepth == d3d9_get_mode_bit(mode.Format))
...
void CDX9Disp::UpdateAntialiasList(int select)
{
#ifdef DX9_ENABLED
	...
	D3DDISPLAYMODE mode;
	if(FAILED(d3d_interface->EnumAdapterModes(selected_adapter, D3DFMT_X8R8G8B8, selected_mode, &mode))) 
	{
	...
	{		
		HRESULT hr = d3d_interface->CheckDeviceMultiSampleType( 
			selected_adapter, 
			D3DDEVTYPE_HAL , 
			mode.Format, FALSE, 
			multisample_info[i].type, NULL );
...
/**
 * Generate textfile version of D3D9 caps
 * Allow user to save it where they want then open it
 */
void CDX9Disp::OnGenCaps() 
{
#ifdef DX9_ENABLED
	...
	// Adapter data
	D3DADAPTER_IDENTIFIER9 identifier;
	if(FAILED(d3d_interface->GetAdapterIdentifier(selected_adapter, 0, &identifier)))
	{
...
#define SPRINTF_NL() i += sprintf(mass_buffer + i, "\n")
		...
		// This outputs all the DX9 caps to text
		i += sprintf(mass_buffer + i, "\n\nD3D9 Caps:\n\n"); 
	...
	char filename[MAX_PATH] = {"D3D9 CAPS "};

	unsigned long len = MAX_PATH - strlen(filename); 
	GetUserName(filename + strlen(filename), &len); 

	FILE *fp = browse_for_and_open_save_file(this->GetSafeHwnd(), filename, "txt", "Save D3D9 CAPS textfile"); 
...
void CDX9Disp::OnApply(int flags)
{
	...
	if(m_dx_initialised_ok == false)
	{
		MessageBox("Cannot apply valid settings, install DX9!", "Error", MB_ICONERROR);
		return;
	}
	...
	D3DDISPLAYMODE mode;
	d3d_interface->EnumAdapterModes(selected_adapter, D3DFMT_X8R8G8B8, mode_num, &mode); 

	char video_card[100];
	int cdepth = d3d9_get_mode_bit(mode.Format);
	...
	if(m_dx_initialised_ok == false)
	{
		MessageBox("This is not an official exe and DX9 has not been compiled in.", "Error", MB_ICONERROR);
		return;
	}
...
void CDX9Disp::OnAllowNonSmodes() 
{
#ifdef DX9_ENABLED
	...
	{
		MessageBox("FS2 DX9 will run under non standard modes but it is not fully supported so expect bugs","Warning",MB_ICONWARNING);
	}
#endif
}
